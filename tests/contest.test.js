...
test('Should fetch other user contest being admin', async () => {
  await request(app)
    .get(`/archery-contest-api/contests/${contestUserOne._id}`)
...
test('Should update other user contest being admin', async () => {
  await request(app)
    .put(`/archery-contest-api/contests/${contestUserOne._id}`)
    .set('Authorization', `Bearer ${userAdmin.tokens[0].token}`)
    .send({
...
test('Should delete user contest being admin', async () => {
  await request(app)
    .delete(`/archery-contest-api/contests/${contestUserOne._id}`)
    .set('Authorization', `Bearer ${userAdmin.tokens[0].token}`)
    .send()
